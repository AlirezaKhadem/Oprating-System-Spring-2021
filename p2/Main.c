...
-----------------------------  SynchTest  ---------------------------------
  ...
  function main ()
      print ("OS Class: project 2\t S.Alireza Khadem 97100398\n")
...
-----------------------------  ProducerConsumer  ---------------------------------
  ...
  var
    ...
    buffer_ready: Semaphore = new Semaphore
    buffer_lock: Mutex = new Mutex
...
-----------------------------  Dining Philosophers  ---------------------------------
  ...
  behavior ForkMonitor
    ...
    method Init ()
      ...
      locks  = new array [5] of Mutex {5 of new Mutex}
      conditions = new array [5] of Condition {5 of new Condition}
    ...
    method test (index: int)
      var 
        right_index: int
        left_index: int

      right_index = (index + 1) % 5
      left_index = ((index - 1) + 5 ) % 5

      if status[right_index] != EATING &&
         status[left_index]  != EATING &&
         status[index] ==  THINKING

          status[index] = EATING
          self.PrintAllStatus ()
          conditions[index].Signal (&locks[index])
