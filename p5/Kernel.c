...
function InitFirstProcess ()
	...
	var 
	  thraed_pointer : ptr to Thread
	thraed_pointer = threadManager.GetANewThread()
	thraed_pointer.Init("UserProgramExample")
	thraed_pointer.Fork(UserProcessExample,0)
endFunction

function UserProcessExample(arg: int)
	var
	   processControlBlock: ptr to ProcessControlBlock
	   file: ptr to OpenFile
	   initUserProgramCounter: int
	...
	initUserProgramCounter = file.LoadExecutable(&processControlBlock.addrSpace) 
	...
	prevStatus = SetInterruptsTo(DISABLED) 
	processControlBlock.addrSpace.SetToThisPageTable()
	currentThread.isUserThread = true 
	BecomeUserThread (initUserStackTop, initUserProgramCounter, initSystemStackTop)
  ...
  behavior ThreadManager
      ...
      method Init ()
        ...
        -- 
          ...
          threadTable = new array [MAX_NUMBER_OF_OPEN_FILES] of Thread {MAX_NUMBER_OF_OPEN_FILES of new Thread}
          ...
          for index = 0 to MAX_NUMBER_OF_PROCESSES - 1
            ...
            th_name = "thread-" + (index + 1)

            th = threadTable[index]
            th.Init (th_name)
            th.status = UNUSED
      ...
      method GetANewThread () returns ptr to Thread
        ...
        -- 
          ...
          new_th = freeList.Remove ()
          new_th.status = JUST_CREATED
...
-----------------------------  ProcessManager  ---------------------------------
  ...
  behavior ProcessManager
      ...
      method Init ()
        ...
        processTable = new array [MAX_NUMBER_OF_PROCESSES] of ProcessControlBlock {MAX_NUMBER_OF_PROCESSES of new ProcessControlBlock}
        ...
        for index = 0 to MAX_NUMBER_OF_PROCESSES - 1
           ...
           processTable[index].status = FREE
  ...
  behavior FrameManager
      ...
      method GetNewFrames (aPageTable: ptr to AddrSpace, numFramesNeeded: int)
          ...
          for index = 0 to numFramesNeeded - 1
            frame_index = framesInUse.FindZeroAndSet()
            frame_address = PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME + (frame_index * PAGE_SIZE)

            aPageTable.SetFrameAddr (index, frame_address)
      ...
      method ReturnAllFrames (aPageTable: ptr to AddrSpace)
          ...
          for index = 0 to aPageTable.numberOfPages - 1
            frame_address = aPageTable.ExtractFrameAddr (index)
            frame_index = (frame_address - PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME) /  PAGE_SIZE

            framesInUse.ClearBit (frame_index)
          endFor
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  ...
  function Handle_Sys_Exec (filename: ptr to array of char) returns int
      ...
      prevStatus = SetInterruptsTo(DISABLED)
      prevStatus = currentThread.myProcess.addrSpace.GetStringFromVirtual(&kernelAddr, filename asInteger, MAX_STRING_SIZE)

      if prevStatus < 0
        return -1 
      endIf

      addressSpace = new AddrSpace
      addressSpace.Init ()

      file = fileManager.Open (&kernelAddr)
      if file == null
        return -1
      endIf
      
      initUserProgramCounter = file.LoadExecutable(&addressSpace)
      if initUserProgramCounter < 0 
        return -1
      endIf
      ...
      currentThread.myProcess.addrSpace = addressSpace 
      currentThread.isUserThread = true

      initUserStackTop = addressSpace.numberOfPages * PAGE_SIZE 
      initSystemStackTop = (&(currentThread.systemStack[SYSTEM_STACK_SIZE-1])) asInteger
      addressSpace.SetToThisPageTable() 
	    BecomeUserThread (initUserStackTop, initUserProgramCounter, initSystemStackTop)
...
-----------------------------  Handle_Sys_Create  ---------------------------------
  ...
  function Handle_Sys_Create (filename: ptr to array of char) returns int
      ...
      var 
      	prevStatus: int
	      addressKernel: array [MAX_STRING_SIZE] of char
	
      prevStatus = currentThread.myProcess.addrSpace.GetStringFromVirtual(&addressKernel, filename asInteger, MAX_STRING_SIZE)
...
-----------------------------  Handle_Sys_Open  ---------------------------------
  ...
  function Handle_Sys_Open (filename: ptr to array of char) returns int
      ...
      oldVar = currentThread.myProcess.addrSpace.GetStringFromVirtual(&addressKernel, filename asInteger, MAX_STRING_SIZE)
